...
const NavBar = () => {
  ...
  return (
    <>
      {/* navbar for desktop*/}
      <div className="border-b border-white/10 px-6 py-4 flex items-center items-stretch justify-between sticky top-0 bg-slate-950/70 backdrop-blur-md z-20 shadow-sm shadow-indigo-500/5">
        <div className="flex items-center">
          <span className="font-extrabold bg-clip-text text-transparent bg-linear-to-r from-white to-slate-400 tracking-tight text-3xl sm:text-5xl">
            Vortex
          </span>
        </div>
        ...
        >
          {isOpen ? <div className="text-white m-0 text-sm">X</div>: (<><span
        ...
        {isOpen ? (
          <div className="flex flex-col gap-4 p-6 bg-[#0d1117] border border-indigo-500/20 rounded-2xl animate-ham z-50 absolute right-0 top-14 text-white">
            ...
            {guest ? <div></div>: <Link to={`/users/profile/${loggedin}`}>Profile</Link>}
            <Link to={"/explore"}>Explore</Link>
            {guest ? <div></div> : <button onClick={logOut}>Logout</button>}
          </div>
        ) : <></>}
        </div>
      </div>
